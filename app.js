// Sistema de Control de Ventas - CON GESTIÓN DE COMPRAS Y AJUSTE MANUAL
class SalesSystem {
    constructor() {
    ...
    updateSummary() {
        try {
            ...
            allCategories.forEach(cat => {
                ...
                categoryReinvestment[cat.id] = {
                    amount: catAmount,
                    purchase: catPurchase,
                    rate: catAmount > 0 ? (catPurchase / catAmount * 100) : 0
                };
            });
            ...
            const chartHTML = `
                <div class="chart-bar">
                    <div class="chart-label">
                        ${category.name}
                        <div class="chart-subtitle" style="font-size: 0.7rem; color: #666;">
                            Reinversión: $${reinvestment.purchase.toFixed(2)} (${reinvestment.rate.toFixed(1)}%)
                        </div>
                    </div>
                    <div class="chart-track">
                        <div class="chart-fill" 
                             id="${category.id}Chart" 
                             style="width: ${percent}%; background: ${category.color}"></div>
                    </div>
                    <div class="chart-value" id="${category.id}Value">${percent.toFixed(1)}%</div>
                </div>
            `;
    ...
    renderProductDetails() {
        ...
        todaySales.forEach(sale => {
            if (!productSummary[sale.productName]) {
                productSummary[sale.productName] = {
                    quantity: 0,
                    amount: 0,
                    purchase: 0,
                    profit: 0,
                        ...
                        ${Object.entries(productSummary).map(([product, data]) => {
                            ...
                            return `
                                <tr>
                                    ...
                                    <td class="${rentPercent >= 0 ? 'text-success' : 'text-danger'}">
    ...
    getReinvestmentStats(date = null) {
        ...
        dailySales.forEach(sale => {
            if (!stats.products[sale.productName]) {
                stats.products[sale.productName] = { sales: 0, amount: 0, reinvestment: 0 };
            }
            stats.products[sale.productName].sales += sale.quantity;
            stats.products[sale.productName].amount += sale.quantity * sale.salePrice;
            stats.products[sale.productName].reinvestment += sale.quantity * sale.purchasePrice;
        });
        
        dailySales.forEach(sale => {
            if (!stats.categories[sale.category]) {
                stats.categories[sale.category] = { sales: 0, amount: 0, reinvestment: 0 };
            }
            stats.categories[sale.category].sales += sale.quantity;
            stats.categories[sale.category].amount += sale.quantity * sale.salePrice;
            stats.categories[sale.category].reinvestment += sale.quantity * sale.purchasePrice;
        });
...
function showAddProductModal() {
    if (window.salesSystem) window.salesSystem.productManager.showAddProductModal();
}

function closeProductModal() {
    if (window.salesSystem) window.salesSystem.productManager.closeProductModal();
}

function filterProducts() {
    const searchTerm = document.getElementById('productSearch');
    if (searchTerm && window.salesSystem) {
        window.salesSystem.productManager.filterProducts(searchTerm.value);
    }
}

function filterInventory() {
    const searchTerm = document.getElementById('inventorySearch');
    if (searchTerm && window.salesSystem) {
        window.salesSystem.productManager.filterInventory(searchTerm.value);
    }
}

function clearTodaySales() {
    if (window.salesSystem) window.salesSystem.clearTodaySales();
}

function refreshProducts() {
    if (window.salesSystem) window.salesSystem.refreshProducts();
}

function loadHistory() {
    if (window.salesSystem) window.salesSystem.historyManager.loadHistory();
}

function hideNotification() {
    if (window.salesSystem) window.salesSystem.uiManager.hideNotification();
}

function showManualSaleModal() {
    if (window.salesSystem) window.salesSystem.historyManager.showManualSaleModal();
}

function closeManualSaleModal() {
    if (window.salesSystem) window.salesSystem.historyManager.closeManualSaleModal();
}

function showPeriodSearchModal() {
    if (window.salesSystem) window.salesSystem.historyManager.showPeriodSearchModal();
}

function closePeriodSearchModal() {
    if (window.salesSystem) window.salesSystem.historyManager.closePeriodSearchModal();
}

function updateManualSaleSummary() {
    if (window.salesSystem) window.salesSystem.historyManager.updateManualSaleSummary();
}

function onManualProductSelectChange(value) {
    if (window.salesSystem) window.salesSystem.historyManager.onManualProductSelectChange(value);
}

function setTodayInHistory() {
    const today = new Date();
    const todayFormatted = today.toISOString().split('T')[0];
    const historyDate = document.getElementById('historyDate');
    if (historyDate) {
        historyDate.value = todayFormatted;
        loadHistory();
    }
}

function showBackupManager() {
    if (window.salesSystem) window.salesSystem.showBackupManager();
}

function showPurchaseModal() {
    if (window.salesSystem && window.salesSystem.purchaseManager) {
        window.salesSystem.purchaseManager.showPurchaseModal();
    }
}

function showInitialBudgetModal() {
    if (window.salesSystem && window.salesSystem.purchaseManager) {
        window.salesSystem.purchaseManager.showInitialBudgetModal();
    }
}

function showCategoryPurchaseStats() {
    if (window.salesSystem && window.salesSystem.purchaseManager) {
        window.salesSystem.purchaseManager.showCategoryPurchaseStats();
    }
}

function showManualBudgetAdjustModal() {
    if (window.salesSystem && window.salesSystem.purchaseManager) {
        window.salesSystem.purchaseManager.showManualBudgetAdjustModal();
    }
}

function closeManualBudgetAdjustModal() {
...
window.addEventListener('error', (event) => {
    ...
    if (notification && window.salesSystem) {
        window.salesSystem.showNotification('Ocurrió un error. Por favor recarga la página.', 'error');
    }
});

// Protección para móviles
if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
    window.addEventListener('beforeunload', (e) => {
        // No mostrar advertencia para no molestar al usuario
        return undefined;
    });
}